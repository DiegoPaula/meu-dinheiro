...
export default new Vuex.Store({
  state: {
    listaDeRegistros: JSON.parse(localStorage.getItem('listaDeRegistros')) || [],
  },
  mutations: {
    acrescentarRegistro: (state, registro) => {
      state.listaDeRegistros.push(registro);
      localStorage.setItem('listaDeRegistros', JSON.stringify(state.listaDeRegistros));
    },
    acrescentarEntrada: (state, entrada) => {
      if (entrada.entrada.categoriaEntrada == "débito") {
        state.listaDeRegistros[entrada.indexRegistro].conteudoRegistro.débitos.push(entrada.entrada)
      } else if (entrada.entrada.categoriaEntrada == "crédito") {
        state.listaDeRegistros[entrada.indexRegistro].conteudoRegistro.créditos.push(entrada.entrada)
      }
      localStorage.setItem('listaDeRegistros', JSON.stringify(state.listaDeRegistros));
    },
